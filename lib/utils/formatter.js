...
export const figmaFormatter = ({ tokens }) => {
  ...
  if (palettes.length < MIN_REQUIRED_PALETTES) {
    logger.error(`Invalid input: must provide at least ${MIN_REQUIRED_PALETTES} color palettes`)
    process.exit(1)
  }
  ...
  for (const palette of palettes) {
    ...
    for (const mode of modes) {
      ...
      if (tokens.length < REQUIRED_TOKENS) {
        logger.error(`Invalid input: color palette '${palette}' mode '${mode}' must include at least ${REQUIRED_TOKENS} tokens`)
        process.exit(1)
      }
    }
  }
