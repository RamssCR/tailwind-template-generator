...
/**
 * Checks if token is any arbitrary value (inside []).
 * @param {string} token - The token to check.
 * @returns {boolean} True if the token is arbitrary, false otherwise.
 * @example
 * isArbitraryToken("bg-[var(--color-primary)]"); // true
 * isArbitraryToken("bg-red-500"); // false
 */
const isArbitraryToken = (token) => /\[.+\]/.test(token)

/**
 * Checks if the token is an arbitrary color token. (inside [var()])
 * @param {string} token
 * @returns {boolean}
 * @example
 * isArbitraryColorToken("bg-[var(--color-primary)]"); // true
 * isArbitraryColorToken("bg-red-500"); // false
 */
const isArbitraryColorToken = (token) => /\[\s*var\(.+\)\s*\]/.test(token)
